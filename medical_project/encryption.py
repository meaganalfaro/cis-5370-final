...
def demo():
    # Demonstrate the file encryption system.
    
    print("\n" + "="*60)
    print("MEDICAL FILE ENCRYPTION SYSTEM - DEMO")
    print("="*60)
    
    # Create the encryptor
    encryptor = MedicalFileEncryptor()
    
    # Creating sample medical files
    print("\nCreating sample medical files...")
    ...
    White Blood Cells: 7200 cells/μL
    
    Interpretation: All values within normal range.
    Physician: Dr. Sarah Smith
    """
    
    with open('blood_test_results.txt', 'w') as f:
        f.write(blood_test_data)
    print("   ✓ Created: blood_test_results.txt")
    ...
    with open('prescription.txt', 'w') as f:
        f.write(prescription_data)
    print("   ✓ Created: prescription.txt")
    ...
    with open('xray_report.txt', 'w') as f:
        f.write(xray_data)
    print("\n\tCreated: xray_report.txt")
    
    input("\nPress Enter to encrypt the files...")
    
    # Encrypting the files
    
    # Encrypt blood test
    record1 = encryptor.encrypt_file(
        'blood_test_results.txt',
        patient_id=1,
        record_type='blood_test'
    )
    
    input("\nPress Enter to encrypt prescription...")
    
    # Encrypt prescription
    record2 = encryptor.encrypt_file(
        'prescription.txt',
        patient_id=1,
        record_type='prescription'
    )
    
    input("\nPress Enter to encrypt X-ray report...")
    
    # Encrypt X-ray
    record3 = encryptor.encrypt_file(
        'xray_report.txt',
        patient_id=1,
        record_type='xray'
    )
    ...
    input("\nPress Enter to view encrypted files database...")
    
    encryptor.list_encrypted_files(patient_id=1)
    
    # Patient wants to view their records
    
    input("\nPress Enter to decrypt files (patient viewing records)...")
    
    print("\nPatient John Doe logs in and requests to view blood test...")
    encryptor.decrypt_file(record_id=1)
    
    input("\Press Enter to decrypt prescription...")
    
    print("\nPatient requests prescription...")
    encryptor.decrypt_file(record_id=2)
    
    # Security demonstration

    
    input("\nPress Enter to see security demonstration...")
    
    encryptor.demonstrate_wrong_key(record_id=1)
    

    # Compare original vs decrypted

    
    input("\nPress Enter to compare original vs decrypted files...")
    
    print("\n" + "="*60)
    print("COMPARING: Original vs Decrypted")
    print("="*60)
    
    with open('blood_test_results.txt', 'r') as f:
        original = f.read()
    
    with open('decrypted_blood_test_results.txt', 'r') as f:
        decrypted = f.read()
    
    print(f"\n✓ Original and decrypted files are identical: {original == decrypted}")
...
1. Encrypted real medical files (not just strings)
2. Each file got its own unique encryption key
3. Stored keys securely in database
4. Successfully decrypted files when authorized
5. Showed that wrong key cannot decrypt
6. Verified original = decrypted (no data loss)

Key Points:
• Files can be ANY format (TXT, PDF, PNG, DOCX, etc.)
• Encryption is transparent (patient doesn't see it)
• Each file = Different key = Better security
• Authentication (login) is separate from encryption

Next: We'll add patient authentication and PIN recovery!
""")
